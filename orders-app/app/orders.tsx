...
import { approveOrder, getOrders, OrderDto } from "../src/api/orders";
import { removeToken } from "../src/auth/tokenStorage";
  ...
  return (
    <SafeAreaView style={{ flex: 1, padding: 12 }}>
      ...
      <FlatList
        ...
        renderItem={({ item }) => (
          <View style={{ borderWidth: 1, borderRadius: 10, padding: 12, marginBottom: 10, gap: 6 }}>
            {/* <Text style={{ fontWeight: "700" }}>ID: {item.orderId}</Text> */}

            <Text>Cliente: {item.customerName ?? item.customerId ?? "-"}</Text>
            ...
            {item.requiresManualApproval ? (
              <View style={{ marginTop: 8, gap: 8 }}>
                <Text style={{ color: "crimson", fontWeight: "600" }}>Requer aprovação manual</Text>
